...
public class GroupService {
    ...
    @Autowired
    private StateMachinePersist<Status, Event, String> stringStateMachinePersist;

    @Autowired
    private StateMachinePersister<Status, Event, String> stateMachinePersister;
        ...
        return Mono.defer(() -> {
            ...
            return Mono.from(stateMachine.stopReactively())
                    .thenEmpty(Flux.fromIterable(stateMachine.getStateMachineAccessor().withAllRegions()).flatMap(region -> {
                                try {
                                    StateMachineContext<Status, Event> context = stringStateMachinePersist.read(machineId);
                                    if (context != null) {
